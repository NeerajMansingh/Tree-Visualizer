...
void rbtpage::insertFixup(RBTreeNode* node) {
    while (node != root && node->parent && node->parent->color == RED) {
        ...
        bool isLeftParent = (node->parent == grandparent->left);
        RBTreeNode* uncle = isLeftParent ? grandparent->right : grandparent->left;
...
void rbtpage::deleteFixup(RBTreeNode*& root, RBTreeNode* node) {
    while (node != root && (node == nullptr || node->color == BLACK)) {
        ...
        RBTreeNode* sibling = isLeftChild ? node->parent->right : node->parent->left;
        ...
        bool bothChildrenBlack = (!sibling->left || sibling->left->color == BLACK) &&
                                 (!sibling->right || sibling->right->color == BLACK);

        if (bothChildrenBlack) {
            sibling->color = RED;
            node = node->parent;
        }
        else {
